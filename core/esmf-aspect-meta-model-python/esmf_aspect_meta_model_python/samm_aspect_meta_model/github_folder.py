...
class GitFolder(GitFile):
    def __init__(
        self, name: str, git_path: str, sha: str, size: int, url: str, local_path: str
    ):
    ...
    def get_folder_from_json(
        json_folder, parent_local_path: str, headers: Optional[dict]
    ) -> "GitFolder":
...
def decoding_url_response(
    parent_folder: GitFolder, url: str, headers: Optional[dict]
) -> GitFolder:
    ...
    for json_object in json_result:
        ...
        if object_type == "dir":
            parent_folder.folders.append(
                GitFolder.get_folder_from_json(
                    json_object, parent_folder.local_path, headers
                )
            )
        if object_type == "file":
