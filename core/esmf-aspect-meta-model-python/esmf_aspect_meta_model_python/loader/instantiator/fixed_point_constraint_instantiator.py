...
from esmf_aspect_meta_model_python.impl.constraints.default_fixed_point_constraint import (
...
class FixedPointConstraintInstantiator(InstantiatorBase[FixedPointConstraint]):
    def _create_instance(self, element_node: Node) -> FixedPointConstraint:
        ...
        scale = self._aspect_graph.value(
            subject=element_node, predicate=self._sammc.get_urn(SAMMC.scale)
        ).toPython()
        integer = self._aspect_graph.value(
            subject=element_node, predicate=self._sammc.get_urn(SAMMC.integer)
        ).toPython()
