...
from esmf_aspect_meta_model_python.base.contraints.range_constraint import (
    RangeConstraint,
)
from esmf_aspect_meta_model_python.loader.instantiator_base import InstantiatorBase
from esmf_aspect_meta_model_python.base.bound_definition import BoundDefinition
from esmf_aspect_meta_model_python.vocabulary.SAMMC import SAMMC
from esmf_aspect_meta_model_python.impl.constraints.default_range_constraint import (
...
class RangeConstraintInstantiator(InstantiatorBase[RangeConstraint]):
    def _create_instance(self, element_node: Node) -> RangeConstraint:
        ...
        lower_bound_definition = self.__get_lower_bound_definition(
            element_node, min_value
        )

        upper_bound_definition = self.__get_upper_bound_definition(
            element_node, max_value
        )
    ...
    def __get_upper_bound_definition(self, element_node, max_value):
        ...
        if upper_bound_definition_value is not None:
            return RangeConstraintInstantiator.__get_bound_definition(
                upper_bound_definition_value
            )
    ...
    def __get_lower_bound_definition(self, element_node, min_value):
        ...
        if lower_bound_definition_value is not None:
            return RangeConstraintInstantiator.__get_bound_definition(
                lower_bound_definition_value
            )

        return BoundDefinition.OPEN if min_value is None else BoundDefinition.AT_LEAST

    def __get_max_value(self, element_node):
        max_value = self._aspect_graph.value(
            subject=element_node, predicate=self._sammc.get_urn(SAMMC.max_value)
        )
    ...
    def __get_min_value(self, element_node):
        min_value = self._aspect_graph.value(
            subject=element_node, predicate=self._sammc.get_urn(SAMMC.min_value)
        )
    ...
    def __get_bound_definition(bound_definition_value: URIRef) -> BoundDefinition:
        ...
        bound_definition_string = bound_definition_value_string[
            bound_definition_start_index:
        ]
