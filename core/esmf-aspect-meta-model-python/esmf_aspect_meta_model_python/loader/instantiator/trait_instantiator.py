...
from esmf_aspect_meta_model_python.impl.characteristics.default_trait import (
    DefaultTrait,
)
...
class TraitInstantiator(InstantiatorBase[Trait]):
    def _create_instance(self, element_node: Node) -> Trait:
        ...
        constraint_subjects = self._aspect_graph.objects(
            subject=element_node, predicate=self._sammc.get_urn(SAMMC.constraint)
        )

        constraints: List[Constraint] = [
            self._model_element_factory.create_element(constraint_subject)
        ...
        base_characteristic = self._get_child(
            element_node, self._sammc.get_urn(SAMMC.base_characteristic), required=True
        )

        return DefaultTrait(
            meta_model_base_attributes, base_characteristic, constraints
        )
