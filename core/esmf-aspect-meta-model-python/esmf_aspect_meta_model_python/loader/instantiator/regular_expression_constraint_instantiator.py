...
from esmf_aspect_meta_model_python.base.contraints.regular_expression_constraint import (
...
class RegularExpressionConstraintInstantiator(
    InstantiatorBase[RegularExpressionConstraint]
):
    def _create_instance(self, element_node: Node) -> RegularExpressionConstraint:
        meta_model_base_attributes = self._get_base_attributes(element_node)
        value = self._aspect_graph.value(
            subject=element_node, predicate=self._samm.get_urn(SAMM.value)
        ).toPython()
