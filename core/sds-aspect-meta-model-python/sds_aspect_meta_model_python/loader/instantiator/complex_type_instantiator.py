...
class ComplexTypeInstantiator(InstantiatorBase[T], metaclass=abc.ABCMeta):
    ...
    def get_extended_element(self, entity_subject: rdflib.URIRef) -> Optional[str]:
        ...
        """
        extended_element_node = self._aspect_graph.value(subject=entity_subject, predicate=self._bamm.get_urn(BAMM.extends))
    ...
    def get_extending_elements(self, entity_subject: rdflib.URIRef) -> List[str]:
        ...
        """
        elements: List[str] = []
        all_element_subjects = self._aspect_graph.subjects(predicate=self._bamm.get_urn(BAMM.extends), object=entity_subject)
