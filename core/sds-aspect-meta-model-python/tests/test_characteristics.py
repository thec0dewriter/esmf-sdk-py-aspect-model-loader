...
from sds_aspect_meta_model_python import AspectLoader, Collection, Enumeration, Quantifiable, Duration, Measurement, StructuredValue, Either
...
def test_loading_aspect_with_collection_with_element_characteristic():
    ...
    assert element_characteristic.urn == "urn:bamm:io.openmanufacturing:characteristic:2.0.0#Text"

    assert element_characteristic.parent_elements[0].urn == "urn:bamm:io.openmanufacturing:aspect-model:TestAspect:1.0.0#TestCollection"
...
def test_loading_aspect_with_quantifiable():
    ...
    assert unit.urn == "urn:bamm:io.openmanufacturing:unit:2.0.0#hertz"
    ...
    assert unit.parent_elements[0].urn == "urn:bamm:io.openmanufacturing:aspect-model:TestAspect:1.0.0#TestQuantifiable"
...
def test_loading_aspect_with_duration():
    ...
    assert duration_characteristic.unit.urn == "urn:bamm:io.openmanufacturing:unit:2.0.0#kilosecond"
...
def test_loading_aspect_with_measurement():
    ...
    assert measurement_characteristic.unit.urn == "urn:bamm:io.openmanufacturing:unit:2.0.0#kelvin"
...
def test_loading_aspect_with_either():
    ...
    assert left.parent_elements[0].urn == "urn:bamm:io.openmanufacturing:aspect-model:TestAspect:1.0.0#TestEither"

    right = either_characteristic.right
    assert right.name == "Boolean"
    assert right.parent_elements[0].urn == "urn:bamm:io.openmanufacturing:aspect-model:TestAspect:1.0.0#TestEither"
