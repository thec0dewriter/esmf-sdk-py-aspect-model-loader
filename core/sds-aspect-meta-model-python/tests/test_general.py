...
from sds_aspect_meta_model_python import AspectLoader, BaseImpl, ComplexType
...
def test_aspect():
    ...
    assert aspect.get_description("en") == "This is a test description"
    assert aspect.urn == "urn:bamm:io.openmanufacturing:aspect-model:TestAspect:1.0.0"
    ...
    assert characteristic.parent_elements[0].urn == "urn:bamm:io.openmanufacturing:aspect-model:TestAspect:1.0.0#testPropertyOne"
    assert characteristic.parent_elements[1].urn == "urn:bamm:io.openmanufacturing:aspect-model:TestAspect:1.0.0#testPropertyTwo"
...
def test_aspect_with_operation():
    ...
    assert aspect.urn == "urn:bamm:io.openmanufacturing.test:1.0.0#AspectWithOperation"
...
def test_aspect_with_operation_no_output():
    ...
    assert aspect.urn == "urn:bamm:io.openmanufacturing.test:1.0.0#AspectWithOperationNoOutput"
...
def test_find_properties_by_name() -> None:
    ...
    assert result[0].urn == "urn:bamm:io.openmanufacturing:aspect-model:TestAspect:1.0.0#testPropertyOne"
    ...
    assert result[0].urn == "urn:bamm:io.openmanufacturing:aspect-model:TestAspect:1.0.0#testPropertyTwo"
    assert len(result[0].preferred_names) == 0
    assert len(result[0].see) == 0
    assert len(result[0].descriptions) == 0
...
def test_find_property_characteristic_by_name() -> None:
    ...
    assert result[0].urn == "urn:bamm:io.openmanufacturing.test:1.0.0#BooleanTestCharacteristic"
    assert len(result[0].preferred_names) == 0
    assert len(result[0].see) == 0
    assert len(result[0].descriptions) == 0
...
def test_find_properties_by_urn() -> None:
    ...
    result = aspect_loader.find_by_urn("urn:bamm:io.openmanufacturing:aspect-model:TestAspect:1.0.0#testPropertyOne")
    assert result is not None
    assert isinstance(result, BaseImpl)
    assert result.name == "testPropertyOne"
    assert result.urn == "urn:bamm:io.openmanufacturing:aspect-model:TestAspect:1.0.0#testPropertyOne"
    assert len(result.preferred_names) == 0
    assert len(result.see) == 0
    assert len(result.descriptions) == 0

    result = aspect_loader.find_by_urn("urn:bamm:io.openmanufacturing:aspect-model:TestAspect:1.0.0#testPropertyTwo")
    assert result is not None
    assert isinstance(result, BaseImpl)
    assert result.name == "testPropertyTwo"
    assert result.urn == "urn:bamm:io.openmanufacturing:aspect-model:TestAspect:1.0.0#testPropertyTwo"
    assert len(result.preferred_names) == 0
    assert len(result.see) == 0
    assert len(result.descriptions) == 0
...
def test_find_property_characteristic_by_urn() -> None:
    ...
    aspect_loader = AspectLoader()
    aspect_loader.load_aspect_model(file_path)
    result = aspect_loader.find_by_urn("urn:bamm:io.openmanufacturing.test:1.0.0#BooleanTestCharacteristic")
    assert result is not None
    assert isinstance(result, BaseImpl)
    assert result.name == "BooleanTestCharacteristic"
    assert result.urn == "urn:bamm:io.openmanufacturing.test:1.0.0#BooleanTestCharacteristic"
    assert len(result.preferred_names) == 0
    assert len(result.see) == 0
    assert len(result.descriptions) == 0
...
def test_load_aspect_from_multiple_files() -> None:
    ...
    aspect_loader = AspectLoader()
    aspect = aspect_loader.load_aspect_model_from_multiple_files([file_path1, file_path2], "urn:bamm:io.openmanufacturing.file_path1:0.0.1#ProductTypes")

    assert aspect.meta_model_version == "2.0.0"
    assert aspect.name == "ProductTypes"
    assert aspect.urn == "urn:bamm:io.openmanufacturing.file_path1:0.0.1#ProductTypes"
    ...
    assert data_type.urn == "urn:bamm:io.openmanufacturing.file_path2:0.0.1#ProductType"
    ...
    assert data_type_properties[0].urn == "urn:bamm:io.openmanufacturing.file_path2:0.0.1#productClass"
    assert data_type_properties[1].urn == "urn:bamm:io.openmanufacturing.file_path2:0.0.1#productSubClass"
    assert data_type_properties[2].urn == "urn:bamm:io.openmanufacturing.file_path2:0.0.1#statisticsGroup"
