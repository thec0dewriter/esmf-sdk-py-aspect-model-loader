...
from sds_aspect_meta_model_python import BaseImpl, AspectLoader
...
def test_find_properties_by_urn() -> None:
    ...
    aspect_loader = AspectLoader()
    aspect_loader.load_aspect_model(file_path)
    element = aspect_loader.find_by_urn("urn:bamm:io.openmanufacturing:aspect-model:TestAspect:1.0.0#testPropertyOne")
    assert element is not None
    assert isinstance(element, BaseImpl)
    assert element.name == "testPropertyOne"
    assert element.urn == "urn:bamm:io.openmanufacturing:aspect-model:TestAspect:1.0.0#testPropertyOne"
    assert len(element.preferred_names) == 0
    assert len(element.see) == 0
    assert len(element.descriptions) == 0

    element = aspect_loader.find_by_urn("urn:bamm:io.openmanufacturing:aspect-model:TestAspect:1.0.0#testPropertyTwo")
    assert element is not None
    assert isinstance(element, BaseImpl)
    assert element.name == "testPropertyTwo"
    assert element.urn == "urn:bamm:io.openmanufacturing:aspect-model:TestAspect:1.0.0#testPropertyTwo"
    assert len(element.preferred_names) == 0
    assert len(element.see) == 0
    assert len(element.descriptions) == 0
...
def test_find_property_chaticaristic_by_urn() -> None:
    ...
    aspect_loader = AspectLoader()
    aspect_loader.load_aspect_model(file_path)
    element = aspect_loader.find_by_urn("urn:bamm:io.openmanufacturing.test:1.0.0#BooleanTestCharacteristic")
    assert element is not None
    assert isinstance(element, BaseImpl)
    assert element.name == "BooleanTestCharacteristic"
    assert element.urn == "urn:bamm:io.openmanufacturing.test:1.0.0#BooleanTestCharacteristic"
    assert len(element.preferred_names) == 0
    assert len(element.see) == 0
    assert len(element.descriptions) == 0
